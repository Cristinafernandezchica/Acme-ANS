...
<acme:form> 
	...
	<jstl:choose>	 
		<jstl:when test="${_command == 'show' && draftMode == false && accepted != null}">
			<acme:button code="assistance-agent.claim.form.button.trackingLogs" action="/assistance-agent/claim/listCompleted?tracking-logId=${id}"/>			
		</jstl:when>
		<jstl:when test="${acme:anyOf(_command, 'show|update|delete|publish') && draftMode == true && accepted != null}">
			<acme:button code="assistance-agent.claim.form.button.trackingLogs" action="/assistance-agent/claim/listUndergoing?tracking-logId=${id}"/>
